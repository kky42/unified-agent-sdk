...
function formatUsage(usage) {
  ...
  return `in=${usage.input_tokens ?? "?"} cache_read=${usage.cache_read_tokens ?? "?"} out=${
    usage.output_tokens ?? "?"
  } total=${usage.total_tokens ?? "?"} ctx=${usage.context_length ?? "?"}`;
}
...
const base = await mkdtemp(join(os.tmpdir(), "unified-agent-sdk-codex-token-usage-"));
...
try {
  for (const p of problems) {
    ...
    const raw = usage?.raw && typeof usage.raw === "object" ? usage.raw : undefined;
    const cumulative = raw && typeof raw.__cumulative === "object" ? raw.__cumulative : undefined;
...
console.table(
  results.map((r) => ({
    ...
    cumulativeIn: r.raw_cumulative?.input_tokens,
    cumulativeOut: r.raw_cumulative?.output_tokens,
    wasReset: r.raw_wasReset,
  })),
);
...
if (process.env.KEEP_CODEX_TOKEN_WORKSPACE === "1") {
